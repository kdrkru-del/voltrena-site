...
export const webDevelopmentData: ServicePageData = {
  ...
  included: {
    base: [
      { label: 'Анализ задачи, поискового спроса и конкурентов' },
      { label: 'Архитектура и UX-проектирование структуры' },
      { label: 'Индивидуальный дизайн без шаблонных рамок' },
      { label: 'Адаптивная верстка на Next.js (отклик до 0.4 сек)' },
      { label: 'Интеграция с amoCRM / Битрикс24 (включено в базу, 0 ₽)' },
      { label: 'Telegram-уведомления дежурному менеджеру' },
      { label: 'Сквозная разметка UTM, YCLID и ClientID' },
      { label: 'Яндекс Метрика с фиксацией конверсий и целей' },
      { label: 'Базовое техническое SEO и микроразметка' },
    ...
    extra: [
      ...
      { label: 'ИИ-квалификация входящих обращений' },
      { label: 'Мультиязычность и локализация под зарубежные рынки' },
    ],
  },
  ...
  faq: [
    {
      ...
      answer:
        'Стоимость фиксируется в договоре до старта работ. В базовую стоимость уже включены верстка на Next.js, скорость загрузки до 0.4 сек, цели Яндекс Метрики и прямая передача лидов в amoCRM или Битрикс24 без скрытых доплат за вебхуки.',
    },
    {
      question: 'Сколько занимает разработка?',
      answer:
        'Конверсионный лендинг запускается за 7–10 рабочих дней. Многостраничный сервисный сайт или B2B-каталог — от 14 до 25 рабочих дней. Первый интерактивный прототип показываем в течение первой недели разработки.',
    },
    {
      question: 'Можно ли создать сайт специально под Яндекс Директ?',
      answer:
        'Да. Мы изначально проектируем посадочные экраны под группы поисковых запросов в Директе, чтобы посетитель попадал в контекст своей фразы, а цена целевого лида снижалась.',
    },
    {
      question: 'Подключаете ли Яндекс Метрику и UTM-метки?',
      answer:
        'Да, это базовый стандарт нашей разработки. Настраиваем счетчик, цели отправки форм и передаем сквозные UTM / YCLID в CRM, чтобы вы знали источник каждой закрытой сделки.',
    },
    {
      question: 'Берете ли отдельную плату за интеграцию с CRM?',
      answer:
        'Нет. Базовая передача заявок в amoCRM или Битрикс24 входит в разработку любого нашего сайта бесплатно (0 ₽). Мы считаем, что сайт без связки с воронкой продаж не имеет смысла.',
    },
    {
      question: 'Можно ли подключить Telegram?',
      answer:
        'Да. Моментальные уведомления о заявках дежурному менеджеру в Telegram входят в базу. Также можем разработать кастомного Telegram-бота для расчета сметы или квалификации клиентов.',
    },
    {
...
export const yandexDirectData: ServicePageData = {
  ...
  intent: {
    ...
    levels: [
      { id: 'high', label: 'Высокое намерение', description: 'Пользователь близок к целевому действию.', terms: ['заказать', 'цена', 'стоимость', 'купить', 'аренда', 'с расчетом'], strength: 100 },
  ...
  workModels: {
    ...
    items: [
      { id: 'launch', title: 'Запуск рекламы', description: 'Для бизнеса, который запускает новое направление или впервые выходит в Яндекс Директ.', items: ['Анализ спроса', 'Коммерческая семантика', 'Структура кампаний', 'Объявления', 'Метрика', 'Запуск'] },
      { id: 'audit', title: 'Экспресс-аудит за 24 часа', description: 'Если кампания уже работает, находим точки скрытого слива бюджета: мусорные фразы, автотаргетинг, сбои разметки и даем 3 четких действия по снижению цены лида.', items: ['Анализ запросов', 'Минус-слова', 'Площадки РСЯ', 'Связка с CRM', 'План оптимизации'] },
      { id: 'management', title: 'Ведение и оптимизация по кассе', description: 'Регулярно анализируем данные, перераспределяем бюджет в пользу фраз, приносящих закрытые сделки в CRM.', items: ['Поисковые запросы', 'Ставки и бюджеты', 'Тестирование объявлений', 'Контроль CPL', 'Окупаемость'] },
    ],
  },
  ...
  faq: [
    { question: 'Сколько стоит настройка Яндекс Директа?', answer: 'Стоимость зависит от количества направлений, регионов, объема семантики и посадочных страниц. Перед стартом фиксируем смету и структуру кампаний.' },
    { question: 'Какой нужен рекламный бюджет?', answer: 'Бюджет рассчитывается индивидуально под регион и конкуренцию на аукционе ставок. На этапе анализа показываем прогноз стоимости клика и рекомендуемый порог входа.' },
    { question: 'Как быстро можно получить аудит действующей рекламы?', answer: 'Экспресс-аудит проводится за 24 часа. Вам достаточно предоставить гостевой доступ на просмотр без права правок — мы подготовим детальный отчет с точками слива бюджета.' },
    { question: 'Когда появятся первые заявки?', answer: 'Первые обращения поступают в первые дни после прохождения модерации. Накопление статистики и снижение цены лида занимают от 2 до 4 недель регулярного ведения.' },
    { question: 'Вы гарантируете количество заявок?', answer: 'Мы гарантируем целевой характер трафика, отключение мусорных запросов и корректность сквозной аналитики. Итоговое количество сделок также зависит от оффера, конкуренции и скорости обработки лидов вашими менеджерами.' },
    { question: 'Можно ли вести рекламу на существующий сайт?', answer: 'Да, но мы предварительно проверяем конверсионность посадочных страниц и скорость загрузки. Если сайт медленный или не отвечает на поисковый запрос, предложим точечные доработки.' },
    { question: 'Вы подключаете Метрику и цели?', answer: 'Да. Настраиваем цели, связываем счетчик с кампаниями и передаем ClientID / YCLID в CRM для сквозного учета сделок.' },
    { question: 'Можно ли подключить CRM?', answer: 'Да. Каждая заявка из Директа сразу передается в amoCRM или Битрикс24 с указанием поисковой фразы и источника.' },
  ],
  ...
  finalCta: {
    title: 'Получите экспресс-аудит рекламы в Директе за 24 часа',
    description: 'Предоставьте гостевой доступ к кабинету без права внесения изменений. Мы проверим кампании и подготовим конкретный отчет с точками роста окупаемости.',
    benefits: ['Проверим поисковые запросы и мусорные площадки', 'Проверим корректность целей в Метрике и связку с CRM', 'Покажем, как снизить стоимость целевого обращения'],
  },
...
export const telegramBotsData: ServicePageData = {
  ...
  types: [
    ...
    },
    {
      ...
      items: ['База ответов на частые вопросы', 'Сбор деталей инцидента', 'Моментальный перевод на оператора'],
    },
    {
      id: 'internal-bot',
      title: 'Internal Bot',
...
export const analyticsData: ServicePageData = {
  ...
  description: 'Связываем рекламу, сайт, заявки и CRM в единый контур сквозной аналитики. Бизнес видит реальную стоимость обращения, качество сделок и чистый ROMI.',
