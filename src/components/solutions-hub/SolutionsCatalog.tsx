        ...
        <div className="mt-14 grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-6xl mx-auto">
          {digitalProducts.map((product, i) => (
            <ScrollReveal key={product.id} delay={i * 50} className={i === digitalProducts.length - 1 ? 'lg:col-span-2' : undefined}>
              <article id={`solution-${product.id}`} className="h-full p-6 md:p-8 rounded-2xl bg-bg-surface border border-border/80 hover:border-accent/50 transition-colors shadow-lg scroll-mt-28 flex flex-col">
                ...
                <h3 className="text-2xl sm:text-3xl font-bold text-text-primary mb-3">{product.title}</h3>
                <p className="text-sm text-text-secondary leading-relaxed mb-5">{product.summary}</p>

                <div className="p-4 rounded-xl bg-bg-primary border border-border mb-5">
                ...
                <div className="mt-auto grid grid-cols-1 sm:grid-cols-2 gap-3 pt-5 border-t border-border/50">
                  <div className="text-xs text-text-secondary">
                    <strong className="text-text-primary block mb-1">После внедрения</strong>
                    {product.outcome}
                  </div>
                  <div className="text-xs text-text-secondary">
                    <strong className="text-accent block mb-1">Можно начать с</strong>
                    {product.minimalStart}
                  </div>
                </div>

                <div className="pt-6 mt-6 border-t border-border/60 flex flex-wrap items-center justify-between gap-3">
                  <span className="text-xs text-text-muted">Отдельная конфигурация под ваш процесс</span>
