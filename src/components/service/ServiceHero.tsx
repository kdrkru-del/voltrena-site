      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          <motion.div variants={itemVariants} className="mb-6 flex justify-center">
            <nav className="font-mono text-[10px] sm:text-xs text-text-muted tracking-wide flex items-center gap-2">
          ...
          <motion.div variants={itemVariants} className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <Button variant="secondary" size="lg" href="#what-we-create">
              {ctaSecondary}
            </Button>
          </motion.div>
          ...
          {heroFlow && heroFlow.length > 0 && (
            <motion.div variants={itemVariants} className="mt-16 sm:mt-24 relative" ref={containerRef}>
              {/* Desktop Flow */}
              <div className="hidden md:flex items-center justify-center max-w-3xl mx-auto relative">
                {heroFlow.map((node, i) => (
                  <React.Fragment key={node.id}>
                    <div className="hero-flow-node relative z-10 border border-accent/40 bg-bg-surface px-6 py-3 rounded-full flex items-center justify-center">
                    ...
                    {i < heroFlow.length - 1 && (
                      <div className="hero-flow-node flex-1 min-w-[40px] flex items-center justify-center relative px-2">
                        <svg width="100%" height="24" className="overflow-visible" preserveAspectRatio="none">
                          <line x1="0" y1="12" x2="100%" y2="12" stroke="currentColor" strokeWidth="1" strokeDasharray="4 4" className="text-accent/30" />
                          <path d="M calc(100% - 6px) 6 L 100% 12 L calc(100% - 6px) 18" fill="none" stroke="currentColor" strokeWidth="1" className="text-accent/30" />
                ...
                {heroFlow.map((node, i) => (
                  <React.Fragment key={node.id}>
                    <ScrollReveal direction="up" delay={i * 100}>
                      <div className="border border-accent/40 bg-bg-surface px-6 py-3 rounded-full flex items-center justify-center min-w-[200px]">
